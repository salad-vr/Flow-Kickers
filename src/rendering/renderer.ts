...
function drawFOV(ctx: CanvasRenderingContext2D, op: Operator, walls: Wall[], grey: boolean) {
  ...
  ctx.beginPath(); ctx.moveTo(op.position.x, op.position.y); ctx.lineTo(poly[0].x, poly[0].y); ctx.stroke();
  ctx.beginPath(); ctx.moveTo(op.position.x, op.position.y); ctx.lineTo(poly[poly.length - 1].x, poly[poly.length - 1].y); ctx.stroke();
  ctx.globalAlpha = 1;
}
